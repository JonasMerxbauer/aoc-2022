...
let smallestDirectorySize = directories["/"].size;
const emptySpace = 70000000 - smallestDirectorySize;
Object.keys(directories).forEach((d) => {
    if (directories[d].size < smallestDirectorySize && directories[d].size + emptySpace > 30000000) {
        smallestDirectorySize = directories[d].size;
    }
});
